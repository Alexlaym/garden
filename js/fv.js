...
$(document).ready(function () {
  ...
  fileListcontroll();
  keyup_form();
  click_submit(); // Переключение режима отображения пароля (показать)
  ...
  $(document).on('change', '.input-country select', function () {
    var $thisMask = $(this).val(),
        ...
        $this = $thisBlock.find('input[data-mask]'); // $('.input-country .jq-selectbox__select-text').css('background-image', $thisImg);
  ...
  $(document).on('click', '.input-file-list span', function () {
    ...
    fileQuantity(object); // console.log($(this).parent().index());
...
function click_submit() {
  $('body').on('click', '.required--sbmt', function (e) {
    ...
    var maskReq = $thisForm.find('.input--mask');
